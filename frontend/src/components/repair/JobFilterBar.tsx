      ...
      <Popover>
        ...
        <PopoverContent align="end" className="w-[320px] space-y-3">
          <div>
            ...
            <select id="jf-status" aria-label="Status" className={selectClass}
              value={filters.status} onChange={(e) => set('status', e.target.value as JobStatus | 'all')}>
          ...
          <div>
            ...
            <select id="jf-tech" aria-label="Technician" className={selectClass}
              value={filters.technicianId} onChange={(e) => set('technicianId', e.target.value)}>
          ...
          <div>
            ...
            <select id="jf-priority" aria-label="Priority" className={selectClass}
              value={filters.priority} onChange={(e) => set('priority', e.target.value as JobPriority | 'all')}>
          ...
          <div>
            ...
            <Input id="jf-device" className="h-9" placeholder="e.g. Laptop"
              value={filters.deviceType} onChange={(e) => set('deviceType', e.target.value)} />
          </div>

          <div>
            <label className={fieldLabel} htmlFor="jf-payment">Payment</label>
            <select id="jf-payment" aria-label="Payment" className={selectClass}
              value={filters.paymentStatus} onChange={(e) => set('paymentStatus', e.target.value as PaymentStatusFilter | 'all')}>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {chips.map((chip) => (
            <span key={chip.key}
              ...
              {chip.label}
              <button aria-label={`Remove ${chip.label}`}
          ...
          <button className="h-7 px-2 text-xs text-[var(--danger)] hover:bg-[var(--danger)]/10 rounded-md transition-colors"
